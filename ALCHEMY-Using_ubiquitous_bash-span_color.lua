local M = {}

-- Parse raw HTML blocks so that nested spans become regular AST elements.
  function M.RawBlock(elem)
    if elem.format == 'html' then
      local html = elem.text
      -- Skip HTML comments entirely
      if html:match('^%s*<!%-%-') then
        return {}
      end

      -- Remove the surrounding <pre> tags generated by asciinema-markdown
      html = html:gsub('<pre>', ''):gsub('</pre>', '')

      local function esc(t)
        t = t:gsub('&#64;', '@'):gsub('&gt;', '>'):gsub('&lt;', '<')
        return t:gsub('([#%%$&_])', '\\%1')
      end

      -- Convert any colourised span tags to LaTeX equivalents across
      -- the entire block.  Process the full block before splitting
      -- into lines so spans that wrap newlines are handled correctly.
      html = html:gsub('<span style="font%-weight:bold;color:#(%x+);?">(.-)</span>',
                       function(c, t)
                         return '\\textbf{\\textcolor[HTML]{' .. c .. '}{' .. esc(t) .. '}}'
                       end)
      html = html:gsub('<span style="color:#(%x+);?">(.-)</span>',
                       function(c, t)
                         return '\\textcolor[HTML]{' .. c .. '}{' .. esc(t) .. '}'
                       end)
      -- Drop empty span tags
      html = html:gsub('<span style="[^"]*"></span>', '')

      -- Split the block on newlines and join with LaTeX line breaks
      local lines = {}
      for line in html:gmatch('[^\n]+') do
        line = line:gsub('&#64;', '@'):gsub('&gt;', '>'):gsub('&lt;', '<')
        table.insert(lines, line)
      end
      local out = table.concat(lines, '\\newline\n')
      return pandoc.RawBlock('latex', out)
    end
  end

-- Convert spans with inline CSS color and weight into LaTeX commands.
function M.Span(elem)
  local style = elem.attributes.style
  if style then
    local color = style:match('color:%s*#(%x+)')
    local weight = style:match('font%-weight:%s*bold')
    if color then
      local text = pandoc.utils.stringify(elem.content)
      local colored = '\\textcolor[HTML]{' .. color .. '}{' .. text .. '}'
      if weight then
        colored = '\\textbf{' .. colored .. '}'
      end
      return pandoc.RawInline('latex', colored)
    end
  end
end

return {M}
