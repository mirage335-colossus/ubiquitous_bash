...
_torServer_SSH_writeCfg() {
	...
	mkdir -p "$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"
	
	rm "$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"/torrc_"${matchingOffsetPorts[0]}" > /dev/null 2>&1
	
	echo "RunAsDaemon 0" >> "$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"/torrc_"${matchingOffsetPorts[0]}"
	echo "DataDirectory "'"'"$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"'"' >> "$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"/torrc_"${matchingOffsetPorts[0]}"
	echo  >> "$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"/torrc_"${matchingOffsetPorts[0]}"
	
	echo "SocksPort 0" >> "$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"/torrc_"${matchingOffsetPorts[0]}"
	echo  >> "$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"/torrc_"${matchingOffsetPorts[0]}"
	...
	do
		...
		echo "HiddenServiceDir "'"'"$scriptLocal"/tor/sshd/"$currentReversePort"/'"' >> "$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"/torrc_"${matchingOffsetPorts[0]}"
		
		echo "HiddenServicePort ""$currentReversePort"" 127.0.0.1:""$currentLocalSSHport" >> "$scriptLocal"/tor/sshd/dd_"${matchingOffsetPorts[0]}"/torrc_"${matchingOffsetPorts[0]}"
