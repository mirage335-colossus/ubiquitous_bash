...
_write_bfq() {
	...
	sudo -n cat << 'CZXWXcRMTo8EmM8i4d' | sudo tee '/etc/modules-load.d/bfq-'"$ubiquitiousBashIDshort"'.conf' > /dev/null
bfq

CZXWXcRMTo8EmM8i4d


	cat << 'CZXWXcRMTo8EmM8i4d' | sudo tee '/etc/udev/rules.d/60-scheduler-'"$ubiquitiousBashIDshort"'.rules' > /dev/null
